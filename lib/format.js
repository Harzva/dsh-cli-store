...
export function formatEntry(entry, platform = process.platform) {
  ...
  return [
    ...
    ...view.installers
      .filter((item) => item.manager === 'manual' && item.url)
      .map((item) => `Official install instructions: ${item.url}`),
...
export function formatInstall(result) {
  ...
  if (result.status === 'manual-install-required') {
    lines.push('Install: follow the official instructions manually; remote install scripts are not auto-executed.')
    if (result.documentation) lines.push(`Documentation: ${result.documentation}`)
    if (result.instructions?.en) lines.push(`Instructions: ${result.instructions.en}`)
    if (result.instructions?.zh) lines.push(`安装说明：${result.instructions.zh}`)
  }

...
const INSTALLER_MANAGERS = new Set(['brew', 'winget', 'cargo', 'npm', 'pnpm', 'manual'])
...
export function validateEntry(entry) {
  ...
  for (const installer of entry.installers) {
    ...
    if (installer.manager === 'manual') {
      assertString(installer.url, `${entry.id}.${installer.id}.url`)
      if (!/^https:\/\//.test(installer.url)) throw new Error(`${entry.id}.${installer.id}.url must use https`)
      if (!installer.instructions || typeof installer.instructions !== 'object') {
        throw new Error(`${entry.id}.${installer.id}.instructions must be an object`)
      }
      assertString(installer.instructions.en, `${entry.id}.${installer.id}.instructions.en`)
      assertString(installer.instructions.zh, `${entry.id}.${installer.id}.instructions.zh`)
      continue
    }
  ...
  return {
    ...
    installers: entry.installers
      ...
      .map((installer) => ({
        ...
        ...(installer.instructions ? { instructions: installer.instructions } : {}),
      })),
  }
}

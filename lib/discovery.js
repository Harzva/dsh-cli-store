...
export async function fetchJson(url, { headers = {}, timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      'user-agent': 'dsh-cli-store/0.2.1 (+https://github.com/Harzva/dsh-cli-store)',
      ...headers,
    },
...
async function discoverCrates(query, { limit, request }) {
  ...
  url.searchParams.set('per_page', String(limit))
  const body = await request(url.toString(), { headers: { 'user-agent': 'dsh-cli-store/0.2.1 (https://github.com/Harzva/dsh-cli-store)' } })
